...
local ui_fov       = menu_tgt:Slider("Cast FOV Limit", 1, 45, 8)
...
local function aim_cast(cmd, pos, bit, id, t, lp, target)
    ...
    local aim_angl = utils.calc_angle(utils.get_camera_pos(), pos)
    local cur_angl = cmd.viewangles

    -- Engagement Limit: Do not aim or shoot if target is outside allowed FOV
    local fov_diff = utils.get_fov(cur_angl, aim_angl)
    if fov_diff > ui_fov:Get() then
        return "failed"
    end

    -- Visibility check
    if lp and target then
        local my_pos = lp:get_origin() + Vector(0, 0, 50)
        if not trace.bullet(my_pos, pos, 10, target) then return "failed" end
    end
...
local function find_best(lp, rm)
    local fast = utils.find_nearest_visible_enemy()
    if fast and fast:valid() and fast:is_alive() and not fast:is_dormant() then
        if (fast:get_origin() - lp:get_origin()):Length() / UNIT_METER <= rm then return fast end
    end
    local best, md = nil, rm
    ...
    for _, e in ipairs(entity_list.by_class_name("C_CitadelPlayerPawn")) do
        if e and e:valid() and e:is_alive() and not e:is_dormant() and e.m_iTeamNum ~= mt then
            local d = (e:get_origin() - mp):Length() / UNIT_METER
            if d <= rm and trace.line(mp + Vector(0, 0, 50), e:get_origin() + Vector(0, 0, 50), 0, lp:get_index(), 0, 0, 0, function() return false end):is_visible_entity(e, d * UNIT_METER) then
                if not best or d < md then best, md = e, d end
            end
        end
    end
    return best
